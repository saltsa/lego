...
func TestDigitalOceanCreateTXTRecord(t *testing.T) {
	...
	mock := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got, want := string(reqBody), `{"type":"TXT","name":"@","data":"foobar"}`; got != want {
	...
	err = doprov.CreateTXTRecord("example.com", "foobar", 0)
	if err != nil {
...
func TestDigitalOceanRemoveTXTRecord(t *testing.T) {
	var requestReceived bool
	...
	doprov.recordIDs["example.com"] = 1234567
	doprov.recordIDsMu.Unlock()

	err = doprov.RemoveTXTRecord("example.com", "foobar", 0)
	if err != nil {
